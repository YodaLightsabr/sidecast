...
${chalk.blue('sideload <github repo> [-v, --verbose]')} - Sideload an extension (GitHub Repo is user/repo#branch) ${chalk.blueBright('[install, i]')}
${chalk.blue('remove <extension name> [-v, --verbose]')} - Remove the files for an extension ${chalk.blueBright('[purge]')}
...
More commands will be added soon!`));
    ...
    'sideload': async ([ghRepo, ...args]) => {
        ...
        if (!ghRepo.startsWith('direct:') && !ghRepo.includes('#')) ghRepo += '#' + json.default_branch;
        let fragments;
        ...
        download(ghRepo, destination, (err) => {
            ...
            (error, stdout, stderr) => {
                stump.success('Extension downloaded! Before the extension can be built, you have to load it first.\n' + `
...
${chalk.bold('2.')} Navigate to ${chalk.blue('~/_sidecast')} and select ${chalk.blue(((fragments.length >= 2 ? fragments[fragments.length - 2] + "/" : '') + fragments[fragments.length - 1]).toLowerCase())}`);
                readline.question(`${chalk.bold('3.')} Press enter to continue`, () => {
                    console.log('');
    ...
    'remove': async ([extension, ...args]) => {
        ...
        try {
            ...
            try {
                let path = os.homedir() + '/_sidecast/' + extension.toLowerCase().split('/').join(':');
                await fs.access(path);
                correctPath = path;
            } catch (err) {
                return stump.error("The extension folder cannot be located. Please try manually deleting it instead.");
            }
        }
        try {
            fs.rm(correctPath, { recursive: true, force: true });
            stump.success('The extension ' + chalk.blue(extension) + ' was removed. You will also have to uninstall it from within Raycast by finding the extension, clicking more actions, and selecting uninstall.');
        } catch (err) {
            if (args.includes('-v') || args.includes('--verbose'))
            return stump.error('There was an error while trying to remove this extension. For more details, use the ' + chalk.blue('--verbose') + '(' + chalk.blue('-v') + ') flag to use verbose mode.')
            else return stump.error('There was an error while trying to remove this extension. Errors: ' + err);
        }
    },
